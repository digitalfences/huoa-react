  ...
  return (
    <div className="hero">
        ...
        <Container col='2'>
          { !!content.hero_title && (
            <div className="hero__content">
              ...
              <Button type='filled' link={ content.button.link ? content.button.link : '/'}>{ content.button.text ? content.button.text : 'Learn More' }</Button>
          </div>
          ) }
        </Container>
      </div>
      ...
      <div className="hero__controls">
        { heroImages.map((el, i) =>
          <span className='hero__selector' data-active={ active === i } onClick={() => setActive(i)} key={ 'selector-' + i }></span>
        ) }
      </div>
    </div>
  );
